<?php

namespace App\Libs\Torrent;

use \Exception;


class Decode
{
    private $encode;

    public function __construct($encode = null)
    {
        if ($encode === null) {
            $encode = new Encode();
        }
        $this->encode = $encode;
    }

    public function decodeFile($file, $strict = false) {
        if (!is_readable($file)) {
            throw new \Exception('File ' . $file . ' does not exist or can not be read.');
        }
        $dictionary = $this->decodeDictionary(file_get_contents($file, true));
        if ($strict) {
            if (!isset($dictionary['announce']) || !is_string($dictionary['announce'])) {
                throw new \Exception('Missing "announce" key.');
            } else if (!isset($dictionary['info']) || !is_array($dictionary['info'])) {
                throw new \Exception('Missing "info" key.');
            }
        }
        return $dictionary;
    }

    public function decode($value)
    {
        if ($value[0] === 'i') {
            return $this->decodeInteger($value);
        } elseif ($value[0] === 'l') {
            return $this->decodeList($value);
        } elseif ($value[0] === 'd') {
            return $this->decodeDictionary($value);
        } elseif (preg_match('/^\d+:/', $value)) {
            return $this->decodeString($value);
        }
        throw new \Exception("Parameter is not correctly encoded.");

    }

    public function decodeInteger($integer)
    {
        if ($integer[0] !== 'i' || (!$ePos = strpos($integer, 'e'))) {
            throw new \Exception("Invalid integer. Integers must start wth 'i' and end with 'e'.");
        }
        $integer = substr($integer, 1, ($ePos - 1));
        $len = strlen($integer);

        if (($integer[0] === '0' && $len > 1) || ($integer[0] === '-' && $integer[1] === '0') || !is_numeric($integer)) {
            throw new \Exception("Invalid integer value.");
        }
        if (PHP_INT_SIZE === 8) {
            return (int) $integer;
        }
        return $integer;
    }

    public function decodeString($string)
    {
        $stringParts = explode(':', $string, 2);

        // The string must have two parts
        if (count($stringParts) !== 2) {
            throw new \Exception("Invalid string. Strings consist of two parts separated by ':'.");
        }

        $length = (int) $stringParts[0];
        $lengthLen = strlen($length);

        if (($lengthLen + 1 + $length) > strlen($string)) {
            throw new \Exception('The length of the string does not match the prefix of the encoded data.');
        }
        return substr($string, ($lengthLen + 1), $length);
    }

    public function decodeList($list)
    {

        if ($list[0] !== 'l') {
            throw new \Exception("Parameter is not an encoded list.");
        }

        $ret = array();

        $length = strlen($list);
        $i = 1;

        while ($i < $length) {
            if ($list[$i] === 'e') {
                break;
            }

            $part = substr($list, $i);
            $decodePart = $this->decode($part);
            $ret[] = $decodePart;
            $i += strlen($this->encode->encode($decodePart));
        }
        return $ret;
    }

    public function decodeDictionary($dictionary)
    {
        if ($dictionary[0] !== 'd') {
            throw new \Exception("Parameter is not an encoded dictionary.");
        }

        $length = strlen($dictionary);
        $ret = array();
        $i = 1;

        while ($i < $length) {
            if ($dictionary[$i] === 'e') {
                break;
            }
            $keyPart = substr($dictionary, $i);
            $key = $this->decodeString($keyPart);
            $keyPartLength = strlen($this->encode->encodeString($key));
            $valuePart = substr($dictionary, ($i + $keyPartLength));
            $value = $this->decode($valuePart);
            $valuePartLength = strlen($this->encode->encode($value));
            $ret[$key] = $value;
            $i += ($keyPartLength + $valuePartLength);
        }
        return $ret;
    }
}
